...
export async function searchProfiles(
  supabase: DB,
  { query, country, team, page = 1, pageSize = 24 }: SearchFilters
): Promise<{ rows: Profile[]; total: number }> {
  const from = (page - 1) * pageSize;
  const to = from + pageSize - 1;

  if (team) {
    // Search by team name: resolve matching team ids first, then intersect
    // with matching profiles (owner or linked roster member).
    const { data: teams } = await supabase.from("teams").select("id").ilike("team_name", `%${team}%`);
    const teamIds = (teams ?? []).map((t) => t.id);
    if (teamIds.length === 0) return { rows: [], total: 0 };

    const [{ data: owners }, { data: members }] = await Promise.all([
      supabase.from("teams").select("owner_id").in("id", teamIds),
      supabase.from("team_members").select("user_id").in("team_id", teamIds).not("user_id", "is", null),
    ]);
    ...
    if (profileIds.length === 0) return { rows: [], total: 0 };

    let q = supabase
      .from("profiles")
      .select("*", { count: "exact" })
      .eq("privacy_public_profile", true)
      .in("id", profileIds);
  ...
  let q = supabase.from("profiles").select("*", { count: "exact" }).eq("privacy_public_profile", true);
  if (query) q = q.ilike("username", `%${query}%`);
  if (country) q = q.eq("country", country);
  const { data, count } = await q.range(from, to).order("created_at", { ascending: false });
  return { rows: (data ?? []) as Profile[], total: count ?? 0 };
