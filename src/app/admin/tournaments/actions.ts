...
function readTournamentForm(formData: FormData) {
  return {
    ...
    featured_bracket_stage: String(formData.get("featured_bracket_stage") || "FULL"),
  ...
  for (let attempt = 0; attempt < 3; attempt++) {
    const { data: row, error } = await supabase
      ...
      .insert({
        ...
        featured_bracket_stage: data.featured_bracket_stage,
...
export async function updateTournament(
  ...
  const { error } = await supabase
    ...
    .update({
      ...
      featured_bracket_stage: parsed.data.featured_bracket_stage,
...
export async function duplicateTournament(tournamentId: string): Promise<ActionResult> {
  ...
  const { error } = await insertWithUniqueSlug(supabase, {
    ...
    featured_bracket_stage: original.featured_bracket_stage ?? "FULL",
